...
class CMCComputer:
    def __init__(self,minimum_featrues=10,method='file'):
        assert method in ['file','sparse','sift'],f'{method} is not supported now...'

        os.makedirs('./cache',exist_ok=True)
        self.cache_path = './cache/affine_masort.pkl'
        self.cache = {}
        self.minimum_feature = minimum_featrues
        self.prev_img = None
        self.prev_desc = None
        self.sparse_flow_parm = dict(
            maxCorners=3000,
            qualityLevel=0.01,
            minDistance=1,
            blockSize=3,
        ...
        if method=='sparse':
            ...
            for f_name in os.listdir('cache/cmc_files/MOTChallenge/'):
                ...
                if "MOT17" in tag:
                    tag = tag+"-FRCNN"
                if tag in self.file_names: #ablation txt is not changed
                    continue
    ...
    def _load_files(self,name):
        ...
        with open(self.file_names[name],"r") as fp:
            for line in fp:
                token = [float(f) for f in line.split("\t")[1:7]]
                A = np.eye(2,3)
                A[0,0] = token[0]
                A[0,1] = token[1]
                A[0,2] = token[2]
    ...
    def _affine_sift(self,frame,mask,tag):
        ...
        if self.prev_desc is None:
            ...
            return A
        if desc.shape[0]<self.minimum_feature or self.prev_desc[1].shape[0]<self.minimum_feature:
            return A
        ...
        for m,n in matches:
            if m.distance<0.7*n.distance:
                good.append(m)
        if len(good)>self.minimum_feature:
            src_pots = np.float32([self.prev_desc[0][m.queryIdx].pt for m in good]).reshape(-1,-1,-1)
            dst_pots = np.float32([kp[0][m.trainIdx].pt for m in good]).reshape(-1,-1,-1)
            A,_ = cv2.estimateAffinePartial2D(src_pots,dst_pots,method=cv2.RANSAC)
        else:
    ...
    def _affine_sparse_flow(self,frame,mask,tag):
        A = np.eye(2,3)
        keypoints = cv2.goodFeaturesToTrack(frame,mask=mask,**self.sparse_flow_parm)
